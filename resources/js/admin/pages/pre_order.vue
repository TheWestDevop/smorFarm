                    ...
                    <div class="col-md-12 ">
                        <div class="card shadow">
                            <div class="card-header border-0">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col m-2 text-center">
                                        <h2 class="mb-0">Customer Pre Orders</h2>
                                    </div>
                                    <div class="col">
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-12">
                                    <br>
                                    <Pagination :paginations="admin.pre_orders.pagination" @paginate="getPreOrders()" :offset="5" />
                                    <br>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    computed: {
        filterOrders() {
            let $this = this
            return $this.admin.pre_orders.data.filter((order) => {
    ...
    methods: {
        ...
        getPreOrders() {
            let $this = this
            get(BASE_URL + `/api/admin/pre/orders/?page=${$this.admin.pre_orders.pagination.current_page}`).then((response) => {
                $this.admin.pre_orders.data = response.data.data;
                $this.admin.pre_orders.pagination.current_page = response.data.current_page;
                $this.admin.pre_orders.pagination.total = response.data.total;
                $this.admin.pre_orders.pagination.per_page = response.data.per_page;
                $this.admin.pre_orders.pagination.last_page = response.data.last_page;
                $this.admin.pre_orders.pagination.from = response.data.from;
                $this.admin.pre_orders.pagination.to = response.data.to;

            });
        },
        deleteorder(id) {
            get(BASE_URL + `/api/delete/pre/order/${id}`).then((response) => {
        ...
        OrderDeliver(id,ticket,user) {
            get(BASE_URL + `/api/pre/order/${id}/delivered`).then((response) => {
                ...
                let message = `Your Pre Order with Ticket ${ticket} Has Been Delivered, Thank You For Your Patronage`;
        ...
        OrderDeliveRevised(id) {
            get(BASE_URL + `/api/pre/order/${id}/delivered/revised`).then((response) => {
